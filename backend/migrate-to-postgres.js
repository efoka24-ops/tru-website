 ...
 * Imports data from data.json into PostgreSQL
...
async function migrateData() {
  ...
  try {
    ...
    console.log(`   - ${data.team?.length || 0} team members`);
    ...
    console.log(`   - ${data.news?.length || 0} news`);
    console.log(`   - ${data.jobs?.length || 0} jobs`);

    // Start transaction
    await client.query('BEGIN');

    // Clear existing data (optional - uncomment if needed)
    // await client.query('TRUNCATE TABLE team CASCADE');
    ...
    console.log('✅ All data imported into PostgreSQL');
